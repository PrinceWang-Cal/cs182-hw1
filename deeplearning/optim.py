...
def rmsprop(x, dx, config=None):
    ...
    cache_old = config.get('cache')
    epsilon = config.get('epsilon')
    lr = config.get('learning_rate')
    cache_new = decay_rate * cache_old + (1-decay_rate) * (dx**2)
    next_x = x - lr * (dx/(np.sqrt(cache_new) + epsilon))
...
def adam(x, dx, config=None):
    ...
    m_new = (1-beta1)*dx + beta1 * m
    m_hat_new = m_new / (1 - (beta1)**(t+1))

    v_new = (1-beta2) * (dx**2) + beta2 * v
    v_hat_new = v_new / (1 - (beta2)**(t+1))

    next_x = x - lr * (m_hat_new/(np.sqrt(v_hat_new)+epsilon))
